...
// アセスメントシートPDFから情報抽出エンドポイント
app.post('/api/extract-assessment', upload.single('assessmentPdf'), async (req, res) => {
    if (!req.file) {
        return res.status(400).json({ error: 'PDFファイルが送信されていません。' });
    }
    ...
    try {
        ...
        const extracted = extractFieldsFromAssessment(text);
        extracted.rawText = text.slice(0, 6000);

        res.json(extracted);
    } catch (err) {
        console.error('Assessment PDF extract error:', err);
        res.status(400).json({ error: 'アセスメントシートPDFの読み取りに失敗しました。' });
    }
});

// アセスメントシートPDFからフィールドを推定抽出
function extractFieldsFromAssessment(text) {
    const result = {
        childStatus: '',
        childName: '',
        childAge: '',
    ...
    // 氏名
    const nameMatch = text.match(/(?:氏\s*名|利用者名|児童名|お名前|名前)\s*[:：\s]\s*(.+)/);
    if (nameMatch) {
        const name = nameMatch[1].trim().split(/[\s\n\t]/)[0].replace(/[\(（].+[\)）]/, '').trim();
        if (name && name.length <= 20) result.childName = name;
    }

    // 年齢
    const ageMatch = text.match(/(?:年齢|年\s*齢)\s*[:：\s]\s*(.+)/) || text.match(/(\d{1,2})\s*歳/);
    if (ageMatch) result.childAge = ageMatch[1].trim().slice(0, 30);

    // 診断名
    const diagMatch = text.match(/(?:診断名|障害名|障がい名|疾患名)\s*[:：\s]\s*(.+)/);
    if (diagMatch) result.childProfile = diagMatch[1].trim().slice(0, 100);

    // 日々の様子・特性・行動（複数パターンから抽出して結合）
    const statusParts = [];
    const statusPatterns = [
        /(?:日々の様子|日常の様子|生活の様子|行動の特徴|行動特性|特記事項|気になる点|気になること|現在の様子|現在の状況|本人の様子|児童の様子|利用者の様子)\s*[:：\s]\s*([\s\S]{1,500}?)(?=\n(?:[\u3000-\u9FFF]{2,}[:：]|\d+[\.\)）]|$))/g,
        /(?:コミュニケーション|対人関係|社会性|言語|運動|認知|感覚|生活面|健康面|行動面)\s*[:：\s]\s*([\s\S]{1,300}?)(?=\n(?:[\u3000-\u9FFF]{2,}[:：]|\d+[\.\)）]|$))/g,
        /(?:好きなこと|興味|得意|苦手|課題)\s*[:：\s]\s*([\s\S]{1,300}?)(?=\n(?:[\u3000-\u9FFF]{2,}[:：]|\d+[\.\)）]|$))/g
    ];
    ...
    for (const pattern of statusPatterns) {
        ...
        while ((match = pattern.exec(text)) !== null) {
            ...
            if (content && content.length > 5) {
                statusParts.push(content);
            }
        }
    }

    if (statusParts.length > 0) {
        result.childStatus = statusParts.join('\n');
    }

    // アセスメント結果（検査結果、所見等）
    const assessParts = [];
    const assessPatterns = [
        /(?:検査結果|発達検査|知能検査|WISC|田中ビネー|新版K式|DQ|IQ|発達指数|発達年齢)\s*[:：\s]\s*([\s\S]{1,400}?)(?=\n(?:[\u3000-\u9FFF]{2,}[:：]|\d+[\.\)）]|$))/g,
        /(?:所見|総合所見|まとめ|総括|評価)\s*[:：\s]\s*([\s\S]{1,400}?)(?=\n(?:[\u3000-\u9FFF]{2,}[:：]|\d+[\.\)）]|$))/g
    ];

    for (const pattern of assessPatterns) {
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const content = match[1].trim();
            if (content && content.length > 5) {
                assessParts.push(content);
            }
        }
    }
