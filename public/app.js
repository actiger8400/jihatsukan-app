...
// ========== 観察支援モード ==========
const openObservationBtn = document.getElementById('openObservationBtn');
const closeObservationBtn = document.getElementById('closeObservationBtn');
const observationModal = document.getElementById('observationModal');
const observationSearch = document.getElementById('observationSearch');
...
async function loadObservationExamples() {
    const params = new URLSearchParams();
    if (currentDomain) params.set('domain', currentDomain);
    const q = observationSearch.value.trim();
    if (q) params.set('q', q);

    try {
        const res = await fetch('/api/goal-examples?' + params.toString());
        const data = await res.json();
        renderObservationList(data);
    } catch (err) {
        observationList.innerHTML = '<div style="color: #d32f2f;">読み込みに失敗しました</div>';
    }
}

function renderObservationList(data) {
    observationCount.textContent = `${data.filtered}件 / 全${data.total}件`;
    observationList.innerHTML = '';
    if (data.examples.length === 0) {
        observationList.innerHTML = '<div style="color: #999; padding: 20px; text-align: center;">該当する目標例がありません</div>';
        return;
    }
    data.examples.slice(0, 300).forEach(ex => {
        const card = document.createElement('div');
        card.className = 'goal-card';
        const text = document.createElement('div');
        text.className = 'goal-text';
        text.textContent = ex.goal;
        const tags = document.createElement('div');
        tags.className = 'goal-domains';
        ex.domains.forEach(d => {
            const tag = document.createElement('span');
            tag.className = 'goal-domain-tag';
            tag.textContent = d;
            tags.appendChild(tag);
        });
        card.appendChild(text);
        card.appendChild(tags);
        card.addEventListener('click', () => insertGoalExample(ex.goal));
        observationList.appendChild(card);
    });
}

function insertGoalExample(goal) {
    const textarea = document.getElementById('childStatus');
    const existing = textarea.value.trim();
    const insertion = existing ? '\n・' + goal : '・' + goal;
    textarea.value = existing + insertion;
    textarea.classList.add('auto-filled');
    setTimeout(() => textarea.classList.remove('auto-filled'), 1500);
...
openObservationBtn.addEventListener('click', () => {
    ...
    loadObservationExamples();
});
closeObservationBtn.addEventListener('click', () => {
    observationModal.classList.add('hidden');
});
observationModal.addEventListener('click', (e) => {
    if (e.target === observationModal) observationModal.classList.add('hidden');
});
...
observationSearch.addEventListener('input', () => {
    ...
    searchTimer = setTimeout(loadObservationExamples, 200);
});
